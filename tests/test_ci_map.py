...
def test_trigger_summary(data: dict[object, object], expected: str) -> None:
...
def test_artifact_names_extract_named_and_default_artifacts() -> None:
    ...
    assert _artifact_names(steps, "actions/upload-artifact") == ("coverage", "(all artifacts)")
    assert _artifact_names(steps, "actions/download-artifact") == ("ruff", "(all artifacts)")
